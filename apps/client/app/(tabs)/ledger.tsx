...
export default function Ledger() {
  ...
  useFocusEffect(
    useCallback(() => {
      let live = true;
      (async () => {
        const [predictions, priors, reinterpretations, bodyStates] = await Promise.all([allPredictions(), allPriors(), allReinterpretations(), allBodyStates()]);
  ...
  if (!s) return <Screen><P muted>Loading…</P></Screen>;
  ...
  return (
    <Screen>
      ...
      {s.byPrior.map(({ prior, calibration, furnace }) => (
        <Card key={prior.id}>
          <P style={{ fontWeight: '600' }}>“{prior.label}”</P>
      ...
      {s.untagged.total > 0 ? (
        <Card>
          <P style={{ fontWeight: '600' }}>Not tied to a rule</P>
          <P>{s.untagged.sentence}</P>
        </Card>
      ) : null}

      <Small>{FRAMING.noStreaks} {FRAMING.frameworkCaveat}</Small>
    </Screen>
  );
}
