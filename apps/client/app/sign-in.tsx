...
export default function SignIn() {
  ...
  return (
    <Screen>
      ...
      {!sent ? (
        <>
          <Field label="Email" autoCapitalize="none" keyboardType="email-address" autoComplete="email" value={email} onChangeText={setEmail} placeholder="you@example.com" />
          <Button title="Send me a code" onPress={send} disabled={busy || !email.includes('@')} />
        </>
      ) : (
        <>
          <Field label="Code from your email" keyboardType="number-pad" value={code} onChangeText={setCode} placeholder="123456" />
