 ...
 * Runs on app foreground, on reconnect, and after every write (debounced).
 * Offline is the normal state; nothing here throws at the UI.
 */
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const push: SyncPush = {
      ...
      expoPushToken: await getMeta('expoPushToken'),
    };
...
/** Wire up reconnect → sync. Call once at app start. */
export function startSyncListeners(): () => void {
  const unsub = NetInfo.addEventListener((state) => {
    if (state.isConnected) requestSync(500);
  });
  return unsub;
}
