...
export default function Predict() {
  ...
  const [crisis, setCrisis] = useState<Awaited<ReturnType<typeof crisisGate>> | undefined>();
  const [err, setErr] = useState<string | null>(null);

  useEffect(() => {
    void activePriors().then(setPriors);
  }, []);
  ...
  const save = async () => {
    ...
    if (!priorId && newPriorLabel.trim().length > 0) {
      const prior: Prior = {
        id: newId(),
        label: newPriorLabel.trim(),
        category: newPriorCategory,
        origin: 'client',
        createdBy: 'client',
        createdAt: now,
        clientUpdatedAt: now,
  ...
  if (crisis?.risk.matched) {
    return (
      <Screen>
        <CrisisCard resources={crisis.risk.resources} onDismiss={back} />
  ...
  return (
    <Screen>
      ...
      <Field
        label="The situation"
        hint="What’s coming up that you’re dreading?"
        value={situation}
        onChangeText={setSituation}
        multiline
        placeholder="Telling my brother I can’t make the trip."
      />
      <Field
        label="What you expect to happen"
        hint="The forecast, in words. Concrete enough that the world can grade it."
        value={expected}
        onChangeText={setExpected}
        multiline
        placeholder="He goes quiet and doesn’t call for weeks."
      />
      ...
      {!priorId ? (
        <>
          ...
          {newPriorLabel.trim() ? (
            <Choice
              label="It’s mostly about…"
              options={PRIOR_CATEGORIES.map((c) => ({ value: c, label: PRIOR_CATEGORY_LABELS[c] }))}
              value={newPriorCategory}
              onChange={setNewPriorCategory}
            />
          ) : null}
        </>
      ) : null}
      ...
      <Scale
        label="How many times would this have to go differently before you’d revise the rule?"
        hint="Optional. Say it now, before the result."
        min={1}
        max={10}
        value={reviseAfterN}
        onChange={setReviseAfterN}
      />
      ...
      {!showBody ? (
        ...
        <>
          ...
          <Choice
            label="Where is it?"
            multi
            options={BODY_CHANNELS.map((c) => ({ value: c, label: BODY_CHANNEL_LABELS[c] }))}
            value={channels}
            onChange={(v) => setChannels(toggle(channels, v))}
          />
          <Scale label="How strong?" min={0} max={10} value={intensity} onChange={setIntensity} />
          <Choice
            label="Words for it"
            multi
            options={BODY_SEED_WORDS.map((w) => ({ value: w, label: w }))}
            value={words}
            onChange={(v) => setWords(toggle(words, v))}
          />
          <Field
            label="What is your body telling you it means?"
            hint="Verbatim. “Heart → something’s wrong.”"
            value={verdict}
            onChangeText={setVerdict}
            placeholder="Heart racing → I’m going to lose it in there."
          />
          <Choice
            label="What’s on the table?"
            hint="Anything that could take the credit for getting through it."
            multi
            options={KIT_ITEMS.map((k) => ({ value: k, label: k.replace('_', ' ') }))}
            value={kit}
            onChange={(v) => setKit(toggle(kit, v))}
          />
        </>
      )}
