...
const ABANDON_LABELS: Record<AbandonReason, string> = {
  situation_did_not_occur: 'It never came up',
  avoided: 'I didn’t go',
  left_early: 'I left before it could happen',
  forgot: 'Forgot',
  other: 'Other',
};
const CREDIT_LABELS: Record<CreditedTo, string> = {
  body: 'My body handled it',
  kit: 'Something I brought',
...
export default function Resolve() {
  ...
  useEffect(() => {
    if (!id) return;
    void getPrediction(id).then(setP);
    void bodyStatesFor(id).then((b) => setHadBodyBefore(b.some((x) => x.phase === 'before')));
  }, [id]);
  ...
  if (done) {
    return (
      <Screen>
        {done.crisis ? <CrisisCard resources={done.crisis.risk.resources} onDismiss={back} /> : null}
        ...
        {done.loud ? (
          <Card loud>
            <H2>Read it back</H2>
            <P>
              Before, you wrote: “{p.expectedOutcome}” — and you were {p.confidence}% sure.
            </P>
            <P>Then: “{actual}”</P>
            <P muted>
              That’s the whole point of writing it down first. The forecast was fixed before the result, so this is a real miss, not a memory of one.
            </P>
          </Card>
        ) : (
          <Card>
            <P>Saved.</P>
            {verdict === 'unclear' || present === 'no' ? (
              <Small>This one won’t be scored — it goes in the “can’t say” column, which is worth talking through with a person.</Small>
            ) : null}
          </Card>
        )}
  ...
  return (
    <Screen>
      ...
      <Choice
        label=""
        options={[
          { value: 'resolve', label: 'It happened (or didn’t)' },
          { value: 'abandon', label: 'I never ran it' },
        ]}
        value={mode}
        onChange={setMode}
      />
      ...
      {mode === 'abandon' ? (
        <>
          <Choice
            label="Why not?"
            hint="No judgment. A prediction never tested just stays open, and it’s useful to know which ones."
            options={ABANDON_REASONS.map((r) => ({ value: r, label: ABANDON_LABELS[r] }))}
            value={abandonReason}
        ...
        <>
          ...
          <Field
            label=""
            hint="What they said and did. Not what they were thinking."
            value={actual}
            onChangeText={setActual}
            multiline
            placeholder="He said okay and asked about next month."
          />
          <Choice
            label="Did the forecast come true?"
            options={OUTCOME_VERDICTS.map((v) => ({ value: v, label: VERDICT_LABELS[v] }))}
            value={verdict}
            onChange={setVerdict}
          />
          ...
          <Choice
            label="Were you there for it?"
            hint="Could you say what actually happened, moment to moment? If it was too big to track, say no — that’s useful too."
            options={[
              { value: 'yes', label: 'Yes' },
              { value: 'no', label: 'It was a blur' },
            ]}
            value={present}
            onChange={setPresent}
          />
          ...
          */}
          ...
          {hadBodyBefore ? (
            <>
              ...
              <Choice
                label="Did you stay past the peak?"
                options={[
                  { value: 'yes', label: 'Yes' },
                  { value: 'no', label: 'Left at the first flinch' },
                ]}
                value={ranPastPeak}
                onChange={setRanPastPeak}
              />
              <Choice
                label="Did what your body predicted arrive?"
                options={VERDICT_ARRIVED.map((v) => ({ value: v, label: v }))}
                value={arrived}
                onChange={setArrived}
              />
              <Choice
                label="What got you through it?"
                hint="Be honest about the credit. It’s the only way this column counts."
                options={CREDITED_TO.map((c) => ({ value: c, label: CREDIT_LABELS[c] }))}
                value={credited}
                onChange={setCredited}
              />
              <Choice
                label="Used anything from the kit?"
                multi
                options={KIT_ITEMS.map((k) => ({ value: k, label: k.replace('_', ' ') }))}
                value={kitUsed}
                onChange={(v) => setKitUsed(kitUsed.includes(v) ? kitUsed.filter((x) => x !== v) : [...kitUsed, v])}
              />
          ...
          {p.exitForecast ? (
            ...
            <Choice
              label="Did you take an exit?"
              hint="Optional."
              options={EXIT_MOVES.map((m) => ({ value: m, label: EXIT_MOVE_LABELS[m] }))}
              value={exitActual}
              onChange={setExitActual}
            />
          )}
          {exitActual && exitActual !== 'none' ? <Field label="What it looked like" value={exitNote} onChangeText={setExitNote} maxLength={80} /> : null}
          <Choice
            label="Your part, if any"
            hint="Optional. Did you do anything that made it more likely to go the way you expected?"
            options={OWN_PART_OPTIONS.map((o) => ({ value: o, label: OWN_PART_LABELS[o] }))}
            value={ownPart}
            onChange={setOwnPart}
          />
          <Field
            label="What did you tell yourself about why it went that way?"
            hint="Optional. This is kept as its own dated note; it can’t change what you wrote above."
            value={reinterp}
            onChangeText={setReinterp}
            multiline
            placeholder="He was only being nice because Mom was there."
          />
