...
```
Task: implement docs/go-live-gate.md items B1, B2, and the enforcement of A1. Branch: feature/audit-mfa-baa. Migration 0009.
...
4. Clinician view: the discount rate and dismissed count appear as rows in the furnace profile block, labelled "Discount rate" and "Dismissed misses".
5. Research: counts_for added to the export allowlist; ims added to the instrument enum with the three subscale keys (negative_expectations, assimilation, cognitive_immunization) in the per-instrument subscale schema. Do not include any IMS item text anywhere.
