...
- **No PHI in email bodies, ever.** Outbound mail already carries the recipient's address and the fact that an account exists at a mental health product, and that association is the protected element — the body is not allowed to add to it. An email may say an action is waiting and link to the app. It may not name a clinician, a client, a session, a date, a prediction, a floor, a protocol, or anything a client wrote. Every message is composed in `packages/api/src/services/email-copy.ts`, whose composers take a link and nothing else, and `packages/api/test/phi-email.test.ts` asserts the rendered text exactly.
- **An audit scoped to this repository establishes what our code does, not what the deployed project permits.** Any claim that something "cannot fire" needs the project configuration as evidence, not the absence of a call site — the public API key means our code is not the only caller. Say which of the two a finding rests on.
...
- **A third-party SERVICE** — anything with an account, an endpoint, or a BAA question — **requires a ruling before adoption.** A **LIBRARY** does not: it runs in our process and is a supply-chain decision rather than a vendor one. But **pin the exact version**, and **name in the PR any dependency that touches credentials, PHI, or outbound network**, so the supply-chain surface is visible rather than inferred.
- **A test can encode a bug as expected behaviour.** When fixing a bug, check whether any existing test depended on the broken behaviour, and **say so in the report** — a test that breaks during a fix is evidence about the test, not only about the fix. The export suite passed *because* runs overwrote each other: it read "the" `predictions.csv`, which only exists when the second run destroys the first. The fix is the same shape as the input-assumption rule below, applied to test infrastructure: the helper now throws unless exactly one run directory exists, so a wrong assumption fails at the helper instead of quietly choosing a file.
- **Prefer naming over counting when reading external or generated data** — CSV columns, `argv`, `split` results, regex groups. Positional access is a coupling to something nobody promised. Adding a column at position 0 broke `row[0]` and left `header.indexOf('created_at')` beside it untouched, in the same test.
- **When a fix exists BECAUSE some external thing behaves a certain way, test that behaviour directly, not just the fix.** Otherwise the reason decays out of the codebase and only the workaround remains — and the day the external thing changes, nobody finds out from a failing test, they find out from a fix that has silently become unnecessary and is now just unexplained code. `config.test.ts` asserts that `new URL()` throws on a Cloud SQL socket string, which is not our code and is exactly the point: it is the premise the validator rests on.
- **A validator is a claim about what real values look like.** Before using a general-purpose one — `.url()`, `.email()`, a regex off the shelf — check it against the value **production** will actually supply, not the one development supplies. The two differ most in exactly the places nothing local can catch. `z.string().url()` is `new URL()`, and a Cloud SQL socket connection string is a libpq URI with an empty authority, so the validator rejected the only connection string production uses — `config()` threw before `listen()`, and Cloud Run reported a port timeout naming nothing useful. Every test passed, because every test used a local TCP URL. Where a parser for the real format exists, **validate with the same one the consumer uses**, so the two agree by construction.
- **A guard's negative test must cover its INPUT assumptions, not only its output behaviour.** Prove it reads what it claims to read — break the input and confirm it notices — before trusting that it fires correctly. `check-skips` was tested for output (it fires when a skip appears, it passes when none does) and shipped reading the wrong file: two suites wrote `.vitest-results.json` into one directory, so it read the 66-test rls run, labelled it as the 290-test api suite, and asserted nothing about the latter. Green throughout. **A guard that trusts a filename is a textual constraint wearing a guard's clothing** — same family as the rule below.
- **Prefer rules that are structural over rules that are textual.** A constraint enforced by a type or a signature fails loudly at the point of change; a constraint enforced by wording fails silently the first time someone edits a string in a hurry. Three of this repository's real guarantees are built that way and none of them depend on anyone remembering: the assistant's structured output has nowhere to put a diagnosis, the sign-in payload has nowhere to put PHI, and an email composer's signature has nowhere to put a client.
- **A Google service is not exempt from that rule.** Before any Google product enters the deploy path, check that it is **both on Google's HIPAA covered-products list and generally available**, and name both checks in the PR. Being Google's, being in the same project, being covered by the BAA, and being GA are four different things. Firebase Hosting is Google's and is not on the list. Cloud Run domain mappings are on a covered product and are a Pre-GA offering, which Google's HIPAA guidance says not to use in connection with PHI. Either check failing is disqualifying on its own. **This extends to Google Workspace**: its BAA covers only services on Google's HIPAA Included Functionality list, and third-party Marketplace add-ons are explicitly outside it — installing into a covered product does not make a thing covered.
- **Any build step whose output depends on path resolution gets an image-level assertion.** Three bugs of this shape have shipped now: the API's docs directory resolved by walking up for `pnpm-workspace.yaml`, which does not exist in a container; Next's standalone bundle not tracing `.next/static`, so every page renders and every stylesheet 404s; and `outputFileTracingRoot` computed from `URL.pathname`, which is `/C:/...` on Windows, so the bundle was never emitted. Each is correct in a checkout and silently wrong in an image, and none fails loudly. Tests run in a checkout, so they cannot catch this class — the assertion has to be on the Dockerfile or the built artifact. See `packages/api/test/image-contents.test.ts`.
...
- **A gate closes on evidence that the procedure ran, never on the existence of the procedure.** **Three states, not two**, and most of what gets called "done" is one of the first two:
  - **written** — a procedure exists, nobody has run it
  - **configured** — the setting is applied, nothing has exercised it
  - **verified** — something ran and the output was observed

  Date each transition. The difference between the second and the third is the whole reason the gate document is worth keeping: a setting that is correct and a setting that is known to work are not the same claim, and only the third survives being asked about.
...
Database. Nothing in the schema, RLS, or migrations is Supabase-specific. You cannot prove that yourself — the instance does not exist, you have no gcloud credentials, and CI cannot hold a downloaded key because `iam.disableServiceAccountKeyCreation` is enforced (CI will need Workload Identity Federation; that is Prompt 2's problem, note it and move on). So split it:

- **You write, blind:** the config path, and `scripts/verify-cloudsql.sh` — one script Daniel runs after he creates the instance, which creates the ledger_api role from packages/api/src/db/rls/000_roles.sql, runs `pnpm db:migrate`, then runs `test:rls` with ALLOW_DESTRUCTIVE_TESTS scoped to that hostname, and prints the case count and any failures. It must refuse to run against a hostname that isn't the Cloud SQL instance, and it must never print a password.
- **Daniel runs it** and pastes you the output. Report the RLS case count as whatever the suite actually holds at that point — do not quote a fixed number; the suite was 24 cases when this prompt was first written and is well past 60 now. Connection from Cloud Run via the Cloud SQL connector or Unix socket — config only, documented in .env.example with the production form of DATABASE_URL and DATABASE_MIGRATE_URL. Give Daniel the exact console steps for the instance, written so they can be followed without judgement calls: instance id, PostgreSQL version, the smallest tier that is sane for this workload, automated backups on, point-in-time recovery on, private IP in us-west1 (note whether this needs a VPC and Private Service Access configured first, and give those steps too if so), deletion protection on, and the password handling — the ledger_api and migration passwords go in Secret Manager, never in a file. State the expected monthly cost of the choices you name.
