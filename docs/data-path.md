...
**The unit is the account, not the row.** A single entry a client deleted on their own is not purged by this job: doing it row by row would mean deleting a parent whose children are still live. How long an individually deleted entry is retained is an open question, not an answered one.
