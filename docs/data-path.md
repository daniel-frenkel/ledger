...
**1b. The origin that serves the app shell.** Both apps are **Cloud Run services** — `app.courageloop.com` serves the client PWA's built files through nginx, `courageloop.com` runs the clinician app's Next.js server. Not Firebase Hosting, and not because of cost or convenience: Firebase Hosting is not on Google's HIPAA covered-products list, Cloud Run is, and Google's guidance is that products not covered by the BAA are not used in connection with PHI. Both apps touch PHI — the clinician app reads client records directly.

Neither origin is a PHI hop. The browser loads the shell from it and then calls `api.courageloop.com` directly; nothing proxies through either. That is the A6 claim, and it is to be confirmed with a network panel after the first deploy rather than assumed.
...
**4. Field-level encryption.** Free-text columns (`*_enc`, type `bytea`) are encrypted in `packages/api/src/crypto` with AES-256-GCM. The data key for each column is derived from `FIELD_ENCRYPTION_KEY` with HKDF, using `table.column` as the info string, so no two columns share a key. The envelope stored in the row is `version(1 byte) || iv(12) || ciphertext || tag(16)`. `key_version` on the row says which root key was used, so rotation is a background job, not a migration. The root key lives in **Google Secret Manager** and is injected into the Cloud Run revision at start (`docs/deploy.md` §2); it is never a plain environment variable on the revision, never in the database, and never in code. It is also the one value here that cannot be regenerated: it decrypts every piece of prose in the system, so losing it loses all of that permanently while the rows survive.
