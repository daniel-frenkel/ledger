...
| B5 | **Key management.** `FIELD_ENCRYPTION_KEY` moves from a `.env` file to the host's secret manager (Google Secret Manager under A3); rotation procedure written down and tested once with `key_version`. | **steps written, not yet run** — `docs/deploy.md` §2 puts it in Secret Manager and never in a plain env var. The rotation rehearsal is still outstanding. |
