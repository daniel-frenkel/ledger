...
A7 → A1 template → A2 and A3 together (Prompt 11 moves auth and the database; Prompt 2 deploys) → A4 → B3 (Prompt 4, done) → B1, B2 (migration 0008, done) → B5, B9 with the deploy → B4, B6, B7, B8, C1, C2 → the first clinician signs A1 → the first invite.
