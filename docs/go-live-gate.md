...
| B2 | **MFA for clinicians.** Email OTP is one factor. Supabase Auth supports TOTP; require enrollment before the first invite is created. Clients stay on email OTP. | **done** — migration 0008; the level is read through `AuthAdmin.assuranceLevel()`, so Prompt 11 swaps the implementation and not the check |
