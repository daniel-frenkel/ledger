...
export async function truncateAll(): Promise<void> {
  ...
  await c.connect();
  await c.query(
    'TRUNCATE stack_goals, clinician_stacks, formulations, assistant_runs, link_invites, prediction_priors, crisis_events, journal_entries, reinterpretations, body_states, predictions, priors, clinician_client_links, devices, users CASCADE',
  );
  await c.end();
}
